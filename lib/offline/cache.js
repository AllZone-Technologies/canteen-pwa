...
const CHECKIN_RESTRICTION = 60 * 60 * 1000; // 1 hour in milliseconds
...
class CacheManager {
  ...
  async addCheckinRecord(employeeId, sourceType) {
    ...
    // Log the checkin for debugging
    console.log(`Added checkin record for ${employeeId} via ${sourceType} at ${record.timestamp}`);
  }
  ...
  async getEmployeeCheckinStatus(employeeId) {
    ...
    console.log(`Checkin status for ${employeeId}:`, {
      isCheckedIn,
      ...
      sourceType: lastCheckin.sourceType
    });

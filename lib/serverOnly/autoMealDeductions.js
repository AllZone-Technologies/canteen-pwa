...
class AutoMealDeductions {
  ...
  // Helper function to convert employee_id strings like "UHPE001" to integers like 1
  convertEmployeeIdToInteger(employeeId) {
    if (typeof employeeId === 'string') {
      // Extract numeric part from strings like "UHPE001", "UHPE019", "UHPE020"
      const match = employeeId.match(/UHPE(\d+)/);
      if (match) {
  ...
  generateCSVContent(mealDeductions) {
    ...
    mealDeductions.forEach((deduction) => {
      ...
      // Convert employee_id to integer (e.g., "UHPE001" -> 1, "UHPE019" -> 19)
      const employeeIdInteger = this.convertEmployeeIdToInteger(deduction.employee_id);

      // Log the conversion for debugging
      if (deduction.employee_id !== employeeIdInteger) {
        console.log(`Converting employee_id: ${deduction.employee_id} -> ${employeeIdInteger}`);
      }

      // Add row to CSV
      csvContent += `${employeeIdInteger},${deduction.wage_type},${formattedAmount},${formattedDate},${deduction.currency}\n`;
    });
  ...
  async processAndSaveMealDeductions() {
    try {
      ...
      console.log("Sample meal deduction data before CSV generation:");
      mealDeductions.slice(0, 5).forEach((deduction, index) => {
        console.log(`  ${index + 1}. Original employee_id: ${deduction.employee_id}, Converted: ${this.convertEmployeeIdToInteger(deduction.employee_id)}`);
      });
